//! Local PTY shell management.
//!
//! Spawns the local host's shell inside a portable-pty and streams its output
//! through the host-provided [`crate::events::EventEmitter`] using the same
//! `ssh-data` / `ssh-status` wire contract as remote sessions.
...
fn emit_data(emitter: &SharedEmitter, id: &str, data: &[u8]) {
    emit(
        emitter,
        "ssh-data",
        &DataPayload {
        ...
        loop {
            match reader.read(&mut buffer) {
                ...
                Ok(n) => {
                    ...
                    if emit_now {
                        emit_data(&read_emitter, &read_id, &data);
                    }
                }
            }
        }
...
pub fn local_shell_ready(
    ...
    for data in pending {
        emit_data(emitter, &id, &data);
    }
    Ok(())
}

fn shell_for(state: &LocalShellState, id: &str) -> Result<Arc<Mutex<Box<dyn Write + Send>>>, String> {
    state
        .lock()
        .map_err(|_| "local shell state poisoned")?
        .shells
...
pub fn local_write(state: &LocalShellState, id: String, data: String) -> Result<(), String> {
...
pub fn local_close(state: &LocalShellState, id: String) -> Result<(), String> {
