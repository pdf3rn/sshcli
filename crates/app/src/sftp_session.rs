//! SFTP session management plus local filesystem helpers.
//!
//! Owns the lifecycle of SFTP connections and the local-directory helpers used
//! by the file tree UI. Emits `sftp-progress` events through the host-provided
...
use crate::session::{map_ssh_error, PASSWORD_REQUIRED};
...
pub async fn sftp_close(state: &SftpState, id: String) -> Result<(), String> {
    let mut manager = state.lock().map_err(|_| "sftp state poisoned")?;
...
pub async fn sftp_pwd(state: &SftpState, id: String) -> Result<String, String> {
    let session = fetch_session(state, &id)?;
    let session = session.lock().await;
...
pub async fn sftp_download(
    ...
    sftp::download(
        ...
        progress_closure(emitter, &id, name, "download"),
    )
    .await
    .map_err(|error| error.to_string())
}
...
pub async fn sftp_upload(
    ...
    sftp::upload(
        ...
        &std::path::PathBuf::from(local),
        &remote,
        progress_closure(emitter, &id, name, "upload"),
    )
    .await
    .map_err(|error| error.to_string())
}

pub async fn sftp_file_exists(state: &SftpState, id: String, path: String) -> Result<bool, String> {
    let session = fetch_session(state, &id)?;
    let session = session.lock().await;
...
pub async fn sftp_mkdir(state: &SftpState, id: String, path: String) -> Result<(), String> {
    let session = fetch_session(state, &id)?;
    let session = session.lock().await;
...
pub async fn sftp_rm_file(state: &SftpState, id: String, path: String) -> Result<(), String> {
    let session = fetch_session(state, &id)?;
    let session = session.lock().await;
...
pub async fn sftp_rm_dir(state: &SftpState, id: String, path: String) -> Result<(), String> {
    let session = fetch_session(state, &id)?;
    let session = session.lock().await;
