//! SSH port-forward (tunnel) management.
...
pub fn tunnel_list(state: &TunnelState) -> Result<Vec<TunnelInfo>, String> {
...
pub async fn tunnel_stop(state: &TunnelState, id: String) -> Result<(), String> {
    let removed = {
        let mut manager = state.lock().map_err(|_| "tunnel state poisoned")?;
